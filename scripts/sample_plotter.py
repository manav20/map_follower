...
class DragHandler(object):
    ...
    def on_release_event(self, event):
        ...
        if self.dragged is not None :
            ...
            index = int(a[-3])
            p.x[index] = old_pos[0] + event.xdata - self.pick_pos[0]
            p.y[index] = old_pos[1] + event.ydata - self.pick_pos[1]
            print(new_pos)
            self.dragged = None
            p.close()
            p.scatter(p.x, p.y)
...
p.x, p.y = random.normal(5, 2, size=(2, 9))
p.labels = [ "%d" % i for i in xrange(p.x.size)]
...
for a,b,l in zip(p.x,p.y, p.labels):
