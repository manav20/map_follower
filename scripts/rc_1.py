...
class Car:
    ...
    def to_Rviz(self,waypoint_x,waypoint_y):
	points = Marker()
	...
	point = Point()
	point.x = self.waypoint_x
	point.y = self.waypoint_y
...
def main():
    ...
    while not rospy.is_shutdown():
        c.to_Rviz(c.waypoint_x, c.waypoint_y)
        c.amclListener()
        rate = rospy.Rate(20)
        while not rospy.is_shutdown():
