...
class waypoint(object):
	...
	def get_way_point(self, msg):
		...
		yaw = math.degrees(euler[2])
                #with open('waypoint_file_race_x.csv', mode = 'a') as waypoint_file_x:
                #   waypoint_writer = csv.writer(waypoint_file_x, delimiter = ',')
                #  waypoint_writer.writerow([msg.pose.position.x])
                #with open('waypoint_file_race_y.csv', mode = 'a') as waypoint_file_y:
                #   waypoint_writer = csv.writer(waypoint_file_y, delimiter = ',')
                #   waypoint_writer.writerow([msg.pose.position.y])
                #with open('waypoint_file_race_yaw.csv', mode = 'a') as waypoint_file_yaw:
                #    waypoint_writer = csv.writer(waypoint_file_yaw, delimiter = ',')
                #    waypoint_writer.writerow([yaw])
                print('X: {}m   Y: {}m   Yaw: {}deg'.format(msg.pose.position.x, msg.pose.position.y, yaw))
